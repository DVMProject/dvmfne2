                            ...
                            */
                            ...
                            if (tgtSystem.FneType == FneType.MASTER)
                            {
                                ...
                                foreach (uint peerId in master.Peers.Keys)
                                    master.SendPeer(peerId, FneBase.CreateOpcode(Constants.NET_FUNC_PROTOCOL, Constants.NET_PROTOCOL_SUBFUNC_NXDN), frame, e.PacketSequence, e.StreamId);
                            }
                            else if (tgtSystem.FneType == FneType.PEER)
                            {
                                FnePeer peer = (FnePeer)tgtSystem.fne;
                                peer.SendMaster(FneBase.CreateOpcode(Constants.NET_FUNC_PROTOCOL, Constants.NET_PROTOCOL_SUBFUNC_NXDN), frame, e.PacketSequence, e.StreamId);
                            }
